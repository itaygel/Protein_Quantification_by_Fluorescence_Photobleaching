...
for j=1:4%namber_of_experiments
...
nnn(j)=round(50*10^(j/3));
[ yt,frame_num ] = build_flur_matrix_p_var(frame_num,10000,nnn(j),1,false,0,753,0 );


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [ yt1,frame_num ] = build_flur_matrix(45,50000,round(0.7*nnn(j)),1-0.85,false,0,750,0 );
% [ yt2,frame_num ] = build_flur_matrix(45,50000,nnn(j)-round(0.7*nnn(j)),1-0.95,false,0,750,0 );
% 
% yt=yt1+yt2;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
stop_frame=0;
    ...
    start_frame=1;
end_frame=frame_num-stop_frame;%40;
f=1;
y0=yt(1);
...
figure(51)
plot(hist_max(:,2),hist_max(:,1),'or')
hold on
plot(hist_max(:,2),hist_max(:,3),'ok')

plot(hist_max(:,2),res(:,1),'xr')
plot(hist_max(:,2),res(:,5),'xk')
legend('ffc' ,'pff', 'ffc med' ,'pff med')
plot([2 7],[2 7])

save ([ 'Two_exp_peter' num2str(fit_for_all), '.mat'])
res