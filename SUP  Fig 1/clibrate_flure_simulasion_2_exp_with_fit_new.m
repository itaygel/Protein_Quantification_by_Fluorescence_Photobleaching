...
[ flur,frame_num ] = build_flur_matrix(6+(ii-1)*2,cell_num,nnn(ii),p,false,0,intencity_for_singel_flur,0 );
[ flur,frame_num ] = build_flur_matrix(6+(ii-1)*2,cell_num,nnn(ii),0.05,false,0,intencity_for_singel_flur,0 );
...
for k=1:frame_num-1
   noise_factor2(k,j)=noise1(k+1)./(flur(1,j)*(damp(k)*(1-damp(k))));  

    
end
%noise_factor2=noise(1:end,:)./(flur(:,1:end-1)*(pp*(1-pp)));  
for k=1:frame_num-1
   noise_factor_pfc(k,j)=noise1(k+1)./(flur(k,j)*(damp_pfc(k)*(1-damp_pfc(k))));  
...
for k=1:frame_num-1
   noise_factor2(k,1:cell_num)=noise(k,:)./(flur(1,:)*(damp(k)*(1-damp(k))));  
...
legend('v var m', 'v var m2' ,'v var m2 artical res no integral' ,'v from fit','v from fit pfc')
xlabel('Frame #')
ylabel('\nu_{est}/\nu')
